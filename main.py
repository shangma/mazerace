...
from service.game import Gamenet, now, seconds_since, \
    ...
    GRAVITY_FACTOR, BOUNCE_FACTOR, DEBUG_OSC
    ...
    def bounce_ball(self, ball):
        if self.collide_widget(ball):
            ...
            if self.vertical:
                ...
                ball.velocity_y = -BOUNCE_FACTOR*vy
                while self.collide_widget(ball): # get out of the parking lot
                    ball.pos[1] += ball.velocity_y
            return True
        return False
...
class MazeraceBall(Widget):
    ...
    def move(self):
        self.velocity = Vector(*self.acceleration) + self.velocity
...
and press [b]OK[/b].""",
    ...
    def check_natural_orientation_callback(self, instance):
        ...
        Clock.schedule_once(lambda dt: self.app.start_client(), 0.1)
...
class MazeraceApp(App):
    ...
    def start_client(self):
        ...
        osc.bind(oscid, self.osc_handle_ready, '/ready')
        ...
        Clock.schedule_interval(lambda dt: self.gui.update(), 0.05)
    ...
    def osc_handle_ready(self, message, *args):
